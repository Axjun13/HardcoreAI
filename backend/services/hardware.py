...
def platformio_ini_for_board(board: str = DEFAULT_BOARD) -> str:
    ...
    if device and not device.supported:
        raise ValueError(
            f"{device.label} is catalog-only: firmware generation is not configured for this board."
        )
    pio_board = device.platformio_board_id if device and device.platformio_board_id else board
    if device and device.pio_platform and device.platformio_board_id:
        framework = (device.frameworks or ["arduino"])[0]
        return "\n".join((
            "; Auto-generated by HardcoreAI â€” edit to retarget.",
            f"[env:{board}]", f"platform = {device.pio_platform}",
            f"board = {pio_board}", f"framework = {framework}",
            f"upload_speed = {device.upload_speed or 115200}", "",
        ))
    if device and device.arch == "avr":
        return _PLATFORMIO_INI_TEMPLATE_AVR.format(
            env=board, board=pio_board, upload_speed=device.upload_speed or 115200,
        )
